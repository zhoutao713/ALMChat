...
public class RegisterActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.easeactivity_register);
	...
	public void register(View view) {
		...
		if (!TextUtils.isEmpty(username) && !TextUtils.isEmpty(pwd)) {
			...
			bu.setInstallId(BmobInstallation.getCurrentInstallation(this).getInstallationId());
...
//					updateUserLocation();
					...
					finish();
					registerInEasemob(username,pwd);
				}
				@Override
	...
	public void registerInEasemob(final String username,final String pwd){
			...
			/*runOnUiThread(new Runnable() {
				public void run() {
					if (!RegisterActivity.this.isFinishing())
						pd.dismiss();
					// 保存用户名
					ALMApplication.getInstance().setUserName(username);
