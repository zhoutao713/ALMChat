...
import com.easemob.chatuidemo.adapter.PersonCenterContentAdapter;
...
public class PersonalFragment extends BaseHomeFragment implements OnClickListener{
	...
	protected void setupViews(Bundle bundle) {
		// TODO Auto-generated method stub
		mUser = ALMApplication.getInstance().getCurrentQiangYu().getAuthor();
	...
	private void initMyPublish() {
		...
		mListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position,
					long id) {
				// TODO Auto-generated method stub
//				ALMApplication.getInstance().setCurrentQiangYu(mQiangYus.get(position-1));
	...
	private void updatePersonalInfo(BUser user) {
		...
		if(user.getAvatar() != null){
			...
			.displayImage(user.getAvatar().getFileUrl(), personalIcon, 
					ALMApplication.getInstance().getOptions(R.drawable.content_image_default),
	 ...
	 * @return
	 */
	private boolean isCurrentUser(BUser user){
		if(null != user){
			BUser cUser = BmobUser.getCurrentUser(mContext, BUser.class);
