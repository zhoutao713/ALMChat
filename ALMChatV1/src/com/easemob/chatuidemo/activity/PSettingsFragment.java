...
import com.xgr.wonderful.util.CacheUtils;
import com.xgr.wonderful.util.Constant;
import com.xgr.wonderful.util.LogUtils;

public class PSettingsFragment extends BaseHomeFragment implements OnClickListener,OnCheckedChangeListener{
	...
	public static PSettingsFragment newInstance(){
		PSettingsFragment fragment = new PSettingsFragment();
		return fragment;
	}
	...
	private void initPersonalInfo(){
		...
		if(user != null){
			...
			if(Constant.SEX_FEMALE.equals(user.getSex())){
	...
	private boolean isLogined(){
		BmobUser user = BmobUser.getCurrentUser(mContext, BUser.class);
		if(user != null){
			return true;
		}
		return false;
		...
		if(avataPath!=null){
			...
			file.upload(mContext, new UploadFileListener() {
				...
				public void onSuccess() {
					...
					BUser currentUser = BmobUser.getCurrentUser(mContext, BUser.class);
